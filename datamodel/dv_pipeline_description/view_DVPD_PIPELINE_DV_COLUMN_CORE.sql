...
select -- own key column
 ...
 where pdt.table_stereotype in ('sat') and pdt.diff_hash_column_name is not null and not is_effectivity_sat and uses_diff_hash and pdt.insert_criteria not in('key') 
 union
 select -- content
 	pdt.pipeline_name 
   ,pdt.table_name
