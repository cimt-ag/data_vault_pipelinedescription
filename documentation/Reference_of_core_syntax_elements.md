...
**relation_names[]**
(optional, only useful on business key fields )
<br>Declares this mapping to be used for specific relations.
The name must be defined as ***relation_name*** at a links parent declaration to the mapped table. Ommitting this property, declares it to be used on the relation without a name (wich is the most occuring case).
Set this zu " * " to use the field on any link relation to the mapped hub. 
...
*Example for 2: "[{"table_name":“raccn_account_hub”, "releation_name":"PARTNER"},{"table_name":“raccn_account_hub”}]"*
...
**relation_name**
(mandatory for every table, declared more then once)
<br>Name of the relation. The name should be usable to extend the hub key column names, since in general the additional hub key columns in the the link will be generated by adding the relation name. The name is referenced by the "relation_name" property of a field, to declare the field to be used for this relation.
