...
The term "partitioned" in this context means, that only a part of the full dataset is delivered and compared. The delivered part is identyfied by the content of some classifing fields and the data set is complete in the boundaries of the partition (e.g. "All contracts of a single company", "all revenues for a month"). Partitions often correlate with the increment pattern of the source.
The partitioning fields can by distributed in various patterns in the data vault. A detailed catalog of scenarions is specified in [Partition Deletion Detection Catalog](./partition_deletion_detection_catalog.md) and must be supported by the dpvd. The range of support by the generators and loading engines can vary.
...
- detection deletion by comparing stage to raw vault, will be defined in 3 variations
    - parameter for full set deletion detecion
	- parameters for most common partioned deletion detection for linear join pathes
	- SQL driven, allowing any kind of complexity/Ruleset
...
- Plan a normal an extra load step for hubs, that are recursivly linked 
...
Stage table hash columns are generated by taking all hash columns of the target model and adding process specific hash columns for target tables with multiple processes.
