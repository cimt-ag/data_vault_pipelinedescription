...
- Phase 1 "Modelling": Gather all necessary knowledge about the source data and design the data vault structure based on that knowledge. 
- Phase 2 "Implementation": Implement database objects (create the tables) and the loading process. Test and deploy the process.

DVPD is designed to gather all information during phase 1. After compiling DVPD and DVPI are the single source of information for phase 2. The following examples focus either on phase 1 or 2. All phase 1 examples are compatible to all phase 2 examples due to the standardization by DVPD/DVPI. 
...
- the model diagram is translated into a DVDP model syntax by the data engineer and merged with the generated field list
...
- a transformation script translates the provided information into a draft DVPD, incuding a data vault data model
...
- in the modelling tool
    - Read and analyze source data structure with the modelling tool
    - Create or extend the data vault model accordingly
    - configure the mapping 
	- add DVPD property data in custom fields, when availalble
...
As mentioned in the introduction, there is no best option, that fits for all projects. It depends on the variety of sources, the available budget, the teamsize and the toolstack already in place or favoured.
...
**It is not recommended to include the DVDP compiler in the execution engine**.
...
Keep in mind, that retesting of released loading processes should be highly avoided. This is why most DWH implementation workflows create packages for deployment, that are decoupled from all future code changes in the development toolset. Feeding the DVDP to a monolithic execution engine puts all implemented pipelines to risk of failiure when the engines gets changed (e.g. update). 
